#!/usr/bin/env python3
"""Count lines of code in tinyqubit directory."""
...
def main():
    tinyqubit_dir = Path('tinyqubit')
    
    results = []
    total_all = 0
    code_all = 0
    
    for py_file in sorted(tinyqubit_dir.rglob('*.py')):
        if '__pycache__' in str(py_file):
            continue
        total, code, comments, blank = count_lines(py_file)
        rel_path = py_file.relative_to(tinyqubit_dir)
