"""Data encoding circuits — the bridge between classical data and quantum states."""
from __future__ import annotations
import numpy as np
from .ir import Circuit, Gate


def angle_encoding(circuit: Circuit, features, wires: list[int], rotation: Gate = Gate.RY) -> None:
    """One rotation gate per wire. Simplest encoding."""
    rot = {Gate.RX: circuit.rx, Gate.RY: circuit.ry, Gate.RZ: circuit.rz}[rotation]
    for w, f in zip(wires, features):
        rot(w, f)


def basis_encoding(circuit: Circuit, features, wires: list[int]) -> None:
    """X gate where feature bit is 1. Binary input only."""
    for w, f in zip(wires, features):
        if f: circuit.x(w)


def amplitude_encoding(circuit: Circuit, features, wires: list[int]) -> None:
...
def iqp_encoding(circuit: Circuit, features, wires: list[int], reps: int = 2) -> None:
    """IQP feature map: H + RZ(x_i) + CZ+RZ(x_i*x_j) per rep."""
    for _ in range(reps):
        for w in wires:
            circuit.h(w)
        for w, f in zip(wires, features):
